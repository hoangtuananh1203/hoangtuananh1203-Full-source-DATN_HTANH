...
recognition.onerror = function () {
  ...
  // Đảm bảo rằng giọng đã được tải sau khi trang được tải
  setTimeout(() => {
    listVoices(); // Gọi lại danh sách giọng nói sau 1 giây
  }, 1000);
...
window.sendQuestion1=async function() {
    ...
    try {
        let response;
            ...
            response = await fetch("http://localhost:8080/api/ChatBot", {
                method: 'POST',
