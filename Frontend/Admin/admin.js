...
window.Sendelasticseach = function () {
  var token = localStorage.getItem("token");
  if (!token) {
    showAlert("Vui lòng đăng nhập trước!");
    return;
  ...
  var api = API_ENDPOINSADMIN.ChatBot + `/daydl`;
  fetch(api, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${token}`,
...
window.RemoteDataelasticsearch = function (id) {
  Swal.fire({
    title: ` Bạn có chắc chắn xoá? `,
    text: "Hành động này không thể hoàn tác!",
    showCancelButton: true,
    ...
    if (result.isConfirmed) {
      ...
      var api = API_ENDPOINSADMIN.ChatBot + `/xoatatca`;
      fetch(api, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
